<template>
  <div class="bg-[hsl(240,10%,16%)] rounded-tr-[90px] font-[Ubuntu] py-17">
    <div
      class="w-[40%] mx-auto text-center text-[hsl(0,100%,100%)] text-[16px] flex flex-col justify-center items-center bg-red-500"
    >
      <div class="h-25 w-25">
        <img src="../../assets/images/logo.svg" alt="company logo" class="object-contain h-full" />
      </div>
      <section>
        <h4 class="font-semibold mt-10 text-[16px] mb-5">Product</h4>
        <ul class="list-none font-light text-[12px]">
          <li class="mb-2 cursor-pointer"><a href="">Overview</a></li>
          <li class="mb-2 cursor-pointer"><a href="">Pricing</a></li>
          <li class="mb-2 cursor-pointer"><a href="">Marketplace</a></li>
          <li class="mb-2 cursor-pointer"><a href="">Features</a></li>
          <li class="mb-2 cursor-pointer"><a href="">Integration</a></li>
        </ul>
      </section>
      <section>
        <h4 class="font-semibold mt-10 text-[16px] mb-5">Company</h4>
        <ul class="list-none font-light text-[12px]">
          <li class="mb-2 cursor-pointer"><a href="">About</a></li>
          <li class="mb-2 cursor-pointer"><a href="">Team</a></li>
          <li class="mb-2 cursor-pointer"><a href="">Blog</a></li>
          <li class="mb-2 cursor-pointer"><a href="">Carrers</a></li>
        </ul>
      </section>
      <section>
        <h4 class="font-semibold mt-10 text-[16px] mb-5">Connect</h4>
        <ul class="list-none font-light text-[12px]">
          <li class="mb-2 cursor-pointer"><a href="">Contact</a></li>
          <li class="mb-2 cursor-pointer"><a href="">Newsletter</a></li>
          <li class="mb-2 cursor-pointer"><a href="">LinkedIn</a></li>
        </ul>
      </section>
    </div>
  </div>
</template>
